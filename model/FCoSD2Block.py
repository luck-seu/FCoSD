...
class FCoSD2(nn.Module):
    def __init__(
        self,
        d_model, # Model input/output dimension D.
        d_inner=0, # Specified internal model dimension.
        d_state=128, # State-space dimension N, expanded from 16 in Mamba-1 to 128.
        d_conv=4, # Kernel size of the 1D convolution.
        conv_init=None,
        expand=2, # Expansion factor E.
        headdim=64, # Head dimension P, i.e. each head has P channels.
        d_ssm=None,  # If not None, we only apply SSM on this many dimensions, the rest uses gated MLP
        ngroups=1,
        A_init_range=(1, 16),
        D_has_hdim=False,
        rmsnorm=True, # Whether to add an extra normalization layer before the final output projection.
        ...
        conv_bias=True, # Whether the convolution layer uses a bias term.
        # Fused kernel and sharding options
        chunk_size=256,
        use_mem_eff_path=True,
        layer_idx=None,  # Absorb kwarg for general module
        process_group=None,
        sequence_parallel=True, # Whether to apply sequence parallelism.
        device=None,
        dtype=None,
        aux_feature_size=0,
    ):
        ...
        # self.d_inner: internal dimension.
        self.d_inner = d_inner // self.world_size if d_inner else (self.expand * self.d_model) // self.world_size
        assert self.d_inner * self.world_size == d_inner if d_inner else self.expand * self.d_model # Ensure divisibility.
        self.headdim = headdim
        # self.d_ssm: total SSM dimension.
        self.d_ssm = self.d_inner if d_ssm is None else d_ssm // self.world_size
        assert ngroups % self.world_size == 0
        self.ngroups = ngroups // self.world_size
        assert self.d_ssm % self.headdim == 0
        # self.nheads: number of heads in the multi-head SSM.
        ...
        self.act = nn.SiLU() # Activation function is fixed to SiLU.
    ...
    def forward(self, u, seqlen=None, seq_idx=None, inference_params=None, B=None, C=None, dt=None):
        ...
        """
        # Either construct SSM parameters from the input, or ensure provided B, C, and dt are not None.
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            if self.no_gen_bcdt: # xBC is x; B and C use the provided parameters.
                x = xBC
            else:
    ...
    def step(self, hidden_states, conv_state, ssm_state, bcdt=(None, None, None)): # hidden_states only have 1 token, seqlen=1
        ...
        if selective_state_update is None:
            ...
            # SSM equation 1: h_t = Ah_{t-1} + Bx_t.
            ssm_state.copy_(ssm_state * rearrange(dA, "b h -> b h 1 1") + dBx) # (B, self.nheads, self.headdim, self.d_state)
            # SSM equation 2: y_t = Ch_t.
            y = torch.einsum("bhpn,bn->bhp", ssm_state.to(dtype), C)
            # Add Dx as the residual term.
            y = y + rearrange(self.D.to(dtype), "h -> h 1") * x
            y = rearrange(y, "b h p -> b (h p)")
            if not self.rmsnorm:
                y = y * self.act(z)  # (B D)
        else:
            # Repeat stored raw parameters A, dt, dt_bias, and D along each head's internal dimension P.
